#!/bin/bash
# ============================================================
# first_setup.sh - ghostfront 初回セットアップスクリプト
...
# 言語設定
# ja: 日本語（SF艦隊風軍人口調のみ、併記なし）
# en: 英語（SF艦隊風軍人口調 + 英訳併記）
...
if [ -f "$BASHRC_FILE" ]; then
    ...
    if ! grep -q "alias css=" "$BASHRC_FILE" 2>/dev/null; then
        # alias が存在しない → 新規追加
        echo "" >> "$BASHRC_FILE"
        echo "# ghostfront aliases (added by first_setup.sh)" >> "$BASHRC_FILE"
